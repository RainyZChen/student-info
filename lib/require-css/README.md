...
```javascript
define(['styles!styles/main'], function() {
  //code that requires the stylesheet: styles/main.styles
});
```
...
map: {
  '*': {
    'css': 'require-styles/styles' // or whatever the path to require-styles is
  }
}
```
...
```javascript
  define(['styles!style', 'styles!page'], function(css) {
    //...
  });
```
...
```javascript
require(['styles!my-styles', 'image!preload-background-image.jpg', 'font!google,families:[Tangerine]']);
```
